  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-in fade-in duration-200">
      <div className="relative w-full max-w-md bg-gradient-to-b from-[#2D040E] to-[#420614] rounded-3xl border border-[#D4AF37]/30 shadow-2xl p-6 sm:p-8 text-white overflow-hidden">
        ...
        {/* Google OAuth Section (Available for both Client and Admin) */}
        <div className="relative my-6 text-center">
          <div className="absolute inset-0 flex items-center">
            <div className="w-full border-t border-white/10" />
          </div>
          <span className="relative px-3 bg-[#38040F] text-[10px] uppercase tracking-widest text-white/50">
            Or authenticate with
          </span>
        </div>

        {/* Container for rendered Google Identity Services Button */}
        <div className="flex flex-col items-center justify-center gap-2">
          <div ref={googleBtnContainerRef} className="min-h-[44px] flex items-center justify-center w-full" />

          {/* Backup trigger button if GIS rendered button is slow or blocked */}
          <button
            type="button"
            onClick={handleManualGoogleClick}
            disabled={loading}
            className="w-full py-2.5 rounded-xl bg-white/10 hover:bg-white/15 border border-white/20 text-white text-xs font-semibold transition-all flex items-center justify-center gap-3 disabled:opacity-50 mt-1"
          >
            ...
            {portalType === 'admin' ? 'Continue with Google (Admin)' : 'Sign in with Google'}
          </button>
        </div>
